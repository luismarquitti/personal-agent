Para integrar o **Google Jules** de forma eficiente ao seu fluxo de trabalho, propunho a criação de um novo Agente Especializado chamado **`jules-issue-architect`**. Este agente será responsável por traduzir especificações de funcionalidades (como as encontradas nas suas *tracks* do Conductor) em Issues do GitHub otimizadas para o processamento assíncrono do Jules.

### Estrutura do Novo Agente (Skill)

O objetivo deste agente é garantir que a Issue contenha contexto suficiente, instruções técnicas precisas e critérios de aceite claros, que são os pilares para que o Jules gere um plano de implementação correto.

Abaixo está a definição da nova Skill que você pode adicionar em `.agents/skills/jules-issue-architect/SKILL.md`:
...
### Como utilizar no seu repositório atual

1.  **Alimente o Agente**: Forneça ao `jules-issue-architect` o conteúdo de um arquivo de especificação, como o `conductor/tracks/rag-engine_20260319/spec.md`.
2.  **Gerar a Issue**: O agente produzirá o corpo da Issue em Markdown.
3.  **Tag Jules**: Ao criar a Issue no GitHub, certifique-se de adicionar a label `jules` ou mencionar `@jules` (conforme sua configuração do Action do Jules) para disparar a execução.
4.  **AGENTS.md**: Recomendo criar um arquivo `AGENTS.md` na raiz do seu projeto. O Jules utiliza este arquivo para entender o contexto global (stack, preferências de linting, comandos de teste) de forma persistente, sem precisar repetir essas informações em todas as Issues.

Com este agente, você transforma o seu "Conductor" em um sistema de despacho automatizado, onde o planejamento gera a Issue e o Jules entrega o Pull Request pronto para revisão.